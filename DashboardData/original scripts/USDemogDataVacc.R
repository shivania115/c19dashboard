...
df <- cdcvacc2 %>% 
  ...
  mutate(demographicVar = ifelse(str_detect(demogLabel, "Age"),"vaccineAge",ifelse(str_detect(demogLabel, "Race"),"vaccineRace",ifelse(str_detect(demogLabel, "Sex"),"vaccineSex","total"))),
...
mergedVacc$popUS <- 327167439


mergedVacc2 <- mergedVacc %>% 
        mutate(demogPop=ifelse(duplicated(mergedVacc$demographic)==TRUE,NA,demogPop),
               percentPop=ifelse(duplicated(mergedVacc$demographic)==TRUE,NA,percentPop)) %>%
  arrange(demographic)


# val <- colnames(mergedVacc)[12:25]
# 
# mergedVacc2[val] <- ifelse(duplicated(mergedVacc$demographic)==TRUE,NA,mergedVacc[,val])
# 
# mergedVacc2[,val] <- apply(mergedVacc[,val], MARGIN=2, function(x) 
#   {ifelse(duplicated(mergedVacc$demographic)==TRUE, NA,x)}) 
# 
# mergedVacc2 <- mergedVacc2 %>% arrange_by(demographic)
# 
# newdata<-cbind(originaldata[,c(-2,-4,-6)], 
#                apply(originaldata[,c(2,4:6)], MARGIN=2, function(x) {ifelse(x==99 | x==-99, NA,x)}))


mergedVacc23 <- na.locf(mergedVacc2, fromFirst = TRUE)

mergedVacc23 <- mergedVacc23 %>% 
  mutate(demogLabel=ifelse(demographicVar=="age",gsub("-"," - ",demogLabel),demogLabel)) %>%
  arrange(demographicVar)   

mergedVacc23$demogPop[mergedVacc23$demographicVar=="vaccineAge" & mergedVacc23$demographic=="<18 years"] <- 53705927 + 19646315 
mergedVacc23$demogPop[mergedVacc23$demographicVar=="vaccineAge" & mergedVacc23$demographic=="75+ years"] <- 15547953 + 6303848
mergedVacc23$demographic[mergedVacc23$demographicVar=="age" & mergedVacc23$demographic=="Unknown years"] <- "Unknown"



rm(cdcvacc2,df,df2,final_data,finalvacc,popdata,prefinal_data,raceUS_final,cdcvacc,deathsTotSex,maxdate_p,maxdate_v,numvar,USTotDose1,USTotDose2,mergedVacc,mergedVacc2)


setwd(box)
write.csv(mergedVacc23,"./DataUpload/RaceData/RaceNation/USVaccineByDemog.csv",na="",row.names=F)
