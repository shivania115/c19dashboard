...
*=========================== daily cases data;
		...
		*14 day change;
		percent14dayCases=(cases-lag14(cases))/lag14(cases);
...
*****************************************************************************;
	PROC IMPORT OUT= WORK.testhosp 
            DATAFILE= "C:\Users\spate41\Box Sync\COVID19_data_shared\Hospitalizations and testing\state_level(char).csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
drop date;
run;
*testing data can be added to the next spreadsheet;
...
PROC EXPORT DATA= WORK.Covidtimeseries_pop 
            OUTFILE= "&datadir.\Yubin\covidtimeseries0.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
series x=date y=caserate7dayfig/group=level  LINEATTRS=( thickness=4);
label caserate7dayfig="Cases per 100,000"  date="Date";
*format excessdeaths percent7.;
run;

/*
proc print data=covidtimeseries_pop;
where (state=13 and county =095);
var date caserate7dayfig;
run;
