...
PROC EXPORT DATA= WORK.Covidtimeseries_pop 
            OUTFILE= "&datadir.\DataUpload\covidtimeseries00.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.names2 
            OUTFILE= "&datadir.\DataUpload\datadescription.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
ods pdf file = "C:\Users\spate41\Box\COVID19_data_shared\Visuals etc\NationalCurves.pdf";
...
title "% change";
proc print data=mergedsocial noobs;
where statename ne " " ;
var statename cases dailycases mean7daycases percent14daycases percent14daydailycases deaths dailydeaths mean7daydeaths percent14daydeaths percent14dayDailyDeaths;
run;

title "Last date in data";
data lastdate;
