...
# SAS ----------
line228_final_hosptest_ts4 <- read_csv(paste0(path_c19dashboard_shared_folder,"/Data/Data Check/Claire_dataUpdate2/line228_final_hosptest_ts4.csv")) 

qc_df <- line228_final_hosptest_ts4
...
rqc02_check1 <- check_by_state %>% 
  dplyr::filter(qc == 2|is.na(qc)) %>% 
  group_by(date) %>% 
  tally()

write.csv(rqc02_check1,paste0(path_c19dashboard_shared_folder,"/Data/Data Check/dashboard data/rqc02_check1.csv"))
...
summary_overall <- summary_check_total(r_df,
                                       ...
                                       r_cols=c("hospDaily","totaltests",
                                                "percentPositive",
                                                "hospAdmissionper100beds","positivePer100K"),
                                       df2_cols=c(
...
summary_state <- summary_check_state(r_df,
                                     ...
                                     r_cols=c("hospDaily","totaltests",
                                              "percentPositive",
                                              "hospAdmissionper100beds","positivePer100K"),
                                     df2_cols=c("hospDaily","totaltests",
                                                "percentPositive",
                                                "hospAdmissionper100beds","positivePer100K")
)


rqc02_check4 <- bind_rows(summary_overall, summary_state) %>% 
  dplyr::filter(r!=qc) %>% 
  dplyr::select(state,fn,name,qc,r) %>% 
  mutate(pct_difference = round(((qc - r)*100)/qc,2))


write.csv(rqc02_check4,paste0(path_c19dashboard_shared_folder,"/Data/Data Check/dashboard data/rqc02_check4.csv"))
